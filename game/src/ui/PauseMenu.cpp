...
PauseMenu::PauseMenu(AudioManager &audio, NetworkClient* network, CheatManager* cheats)
    : _audio(audio)
    , _network(network)
    , _cheats(cheats)
{
    try {
        ...
        std::cerr << "[PauseMenu] FATAL: " << e.what() << std::endl;
        throw;
    }
...
void PauseMenu::onResize(sf::Vector2u winSize) {
  ...
  unsigned int cheatSize =
      static_cast<unsigned int>(clampf(h * 0.035f, 16.f, 40.f));

  float centerX = w / 2.f;
  float centerY = h / 2.f;
  float spacing = clampf(h * 0.10f, 55.f, 110.f);
  float cheatSpacing = clampf(h * 0.07f, 40.f, 70.f);
  ...
  if (isSoloPlayer() && _cheats) {
    float cheatStartY = centerY + spacing + cheatSpacing;

    setupText(_cheatTitle, "--- CHEATS ---", itemSize, {centerX, cheatStartY});

    setupText(_doubleDamageText,
              getCheatButtonText("Degats x2", _cheats->isDoubleDamageActive()),
              cheatSize, {centerX, cheatStartY + cheatSpacing * 0.7f});

    setupText(_invincibilityText,
              getCheatButtonText("Invincibilite", _cheats->isInvincibilityActive()),
              cheatSize, {centerX, cheatStartY + cheatSpacing * 1.4f});

    setupText(_doubleFireRateText,
              getCheatButtonText("Vitesse de tir x2", _cheats->isDoubleFireRateActive()),
              cheatSize, {centerX, cheatStartY + cheatSpacing * 2.1f});
  }
...
bool PauseMenu::isSoloPlayer() const {
  if (!_network) return false;

  auto lobbyStatus = _network->getLobbyStatus();
  return lobbyStatus.players_connected == 1;
}

sf::String PauseMenu::getCheatButtonText(const sf::String& label, bool active) const {
...
void PauseMenu::updateHover(const sf::RenderWindow &window) {
  ...
  if (isSoloPlayer() && _cheats) {
    _cheatTitle.setFillColor(sf::Color(255, 255, 100));

    auto setCheatHover = [&](sf::Text &t, bool active) {
      if (isMouseOver(t, mousePos)) {
        t.setFillColor(_hoverColor);
      } else {
        t.setFillColor(active ? _cheatOnColor : _cheatOffColor);
      }
    };

    setCheatHover(_doubleDamageText, _cheats->isDoubleDamageActive());
    setCheatHover(_invincibilityText, _cheats->isInvincibilityActive());
    setCheatHover(_doubleFireRateText, _cheats->isDoubleFireRateActive());
  }
...
PauseMenuAction PauseMenu::handleEvent(const sf::Event &event) {
  ...
  if (event.type == sf::Event::MouseButtonPressed &&
    ...
    if (isSoloPlayer() && _cheats) {
      if (isMouseOver(_doubleDamageText, mousePos)) {
        return PauseMenuAction::TOGGLE_DOUBLE_DAMAGE;
      }
      if (isMouseOver(_invincibilityText, mousePos)) {
        return PauseMenuAction::TOGGLE_INVINCIBILITY;
      }
      if (isMouseOver(_doubleFireRateText, mousePos)) {
        return PauseMenuAction::TOGGLE_DOUBLE_FIRE_RATE;
      }
    }
  }
...
void PauseMenu::render(sf::RenderWindow &window) {
  ...
  if (isSoloPlayer() && _cheats) {
    window.draw(_cheatTitle);
    window.draw(_doubleDamageText);
    window.draw(_invincibilityText);
    window.draw(_doubleFireRateText);
  }
